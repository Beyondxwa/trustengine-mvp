-- ============================================
-- Fix qr_sessions schema to match create-qr-session
-- Removes created_by/status/jwt_token; adds token/session_id/is_used
-- ============================================
...
-- Service-role only (edge functions). Deny all direct client access.
CREATE POLICY qr_sessions_service_only ON qr_sessions
  FOR ALL
  USING (false)
  WITH CHECK (false);

-- Restore feedback FK only if that table exists
DO $$
BEGIN
  IF EXISTS (
